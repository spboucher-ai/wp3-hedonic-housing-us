...
Standard errors are computed using the HC3 estimator \citep{mackinnon1985some}.

\subsection{Quantile Regression}
\label{sec:qr_methodology}

Quantile regression \citep{koenker1978regression} models the $\tau$-th conditional quantile:
\begin{equation}
...
XGBoost \citep{chen2016xgboost} fits an additive ensemble of regression trees by sequentially minimizing a regularized loss function:
\begin{equation}
...
We also report results for Ridge regression ($\alpha = 1$), Lasso ($\alpha = 0.001$), Elastic Net, Random Forest (500 trees, depth 20), and OLS with state fixed effects.
